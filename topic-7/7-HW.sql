...
SELECT
	...
	(SELECT name FROM products WHERE id = orders_products.product_id) AS product_name,
...
-- 7.3. (по желанию) Пусть имеется таблица рейсов flights (id, from, to) и таблица городов cities (label,
...
ON
	flights.from_city = cities_from.label
JOIN
	cities AS cities_to
ON
	flights.to_city = cities_to.label
ORDER BY id;