...
CREATE TEMPORARY TABLE time_created (id INT UNSIGNED, created_at DATETIME, updated_at DATETIME);
INSERT INTO time_created SELECT id, STR_TO_DATE(created_at, '%d.%m.%Y %h:%i'), STR_TO_DATE(updated_at, '%d.%m.%Y %h:%i') FROM users;
UPDATE users SET created_at = NULL, updated_at = NULL;
ALTER TABLE users 
	MODIFY COLUMN created_at DATETIME,
	MODIFY COLUMN updated_at DATETIME;
UPDATE users SET created_at = (SELECT created_at FROM time_created WHERE time_created.id = users.id);
UPDATE users SET updated_at = (SELECT updated_at FROM time_created WHERE time_created.id = users.id);
DROP TABLE time_created;

SELECT * FROM time_created LIMIT 10;
...
-- Вариант GB
-- SELECT * FROM media WHERE id IN(5, 1, 2) ORDER BY FIELD(id, 5, 1, 2);
